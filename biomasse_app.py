...
ADMIN_PIN = "8319"  # Code-geschützte Admin-Aktion
DATA_ROOT = "data"
...
LOGO_PATH = "logo.png"   # optional; wenn vorhanden, wird es verwendet

# Farben (heller für bessere Lesbarkeit)
THEME_ACCENT = "#198754"  # grün
TEXT_MUTED = "#4b5563"
...
st.markdown(
    ...
    <style>
      ...
      .accent {{ color: {THEME_ACCENT}; font-weight: 700; }}
      ...
      .stTabs [aria-selected="true"] {{
        border-bottom: 2px solid {THEME_ACCENT} !important;
      }}
      .stButton>button {{
        border-radius: 10px;
        border: 1px solid rgba(0,0,0,.15);
      }}
      hr.soft {{ border: none; border-top: 1px solid rgba(0,0,0,.1); }}
    </style>
    """,
...
def safe_load_logo_bytes():
    """Logo-Bytes zurückgeben, wenn Bild gültig; sonst None."""
    if not os.path.exists(LOGO_PATH):
        return None
    try:
        with open(LOGO_PATH, "rb") as f:
            data = f.read()
        Image.open(io.BytesIO(data)).verify()  # Validierung
        return data
    except Exception:
        return None
...
def send_email(subject: str, body: str, to: list, attachments: list = None):
    ...
    Erwartete Secrets (Streamlit Cloud → Settings → Secrets):
    [smtp]
    host="smtp.gmail.com"
    port=465
    ...
    try:
        ...
        st.info("ℹ️ SMTP nicht konfiguriert – Mailversand wird übersprungen.")
        return False
...
def auth_tabs():
    ...
    # Neuanmeldung (Haftungsausschluss + Unterschrift + PDF + Mail)
    with tab_register:
...
def supplier_area(supplier_id: str):
    ...
    with tab_lieferschein:
        ...
        if my_c.empty:
            st.info("Bitte zuerst Kunden anlegen (Reiter 'Kunden').")
        else:
            ...
            if my_mats.empty:
                st.info("Für diesen Kunden sind noch keine Materialien hinterlegt (Reiter 'Materialien').")
            else:
                m_name = st.selectbox("Material", my_mats["name"].tolist(), key="dlv_mat")
                m_row = my_mats.loc[my_mats["name"] == m_name].iloc[0]
                unit = m_row["einheit"]  # Einheit fix aus dem Material
                st.write(f"**Einheit:** {unit}")
                price = st.text_input("Preis/Einheit (€)", value=str(m_row["preis"]), key="dlv_price")

                # Eingabelogik:
                # - Bei m³: NUR Menge sichtbar
                # - Bei kg/t: NUR Voll/Leer sichtbar
                if unit == "m³":
                    amount = st.text_input("Menge (m³)", key="dlv_amount_m3")
                    voll = ""
                    leer = ""
                else:
                    st.caption("Bitte Voll/Leer eingeben. Bei 't' wird automatisch /1000 gerechnet.")
                ...
                if st.button("Lieferschein speichern (PDF & E-Mail)", key="btn_dlv_save"):
                    ...
                    try:
                        if unit == "m³":
                            if not amount.strip():
                                st.error("Bitte Menge in m³ eingeben.")
                                return
                            qty = float(amount.replace(",", "."))
                        else:
                            if not (voll.strip() and leer.strip()):
                                st.error("Bitte Voll und Leer (kg) eingeben.")
                                return
                    ...
                    # Mail an Lieferant + Kunde (falls vorhanden)
